...
if ( ! function_exists( 'searchwp_plugin_activate' ) ) {
	...
	function searchwp_plugin_activate( $network_wide = false ) {
		...
		// Minimum MySQL version requirement.
		if ( version_compare( $wpdb->db_version(), '5.5.3', '<' ) ) {
			require_once ABSPATH . '/wp-admin/includes/plugin.php';
			deactivate_plugins( __FILE__ );
			wp_die( esc_html( __( 'SearchWP requires MySQL 5.5.3 or higher and as a result has been deactivated. Please contact your host to upgrade MySQL before activating this plugin.', 'searchwp' ) ) . ' <a href="' . esc_url( admin_url( 'plugins.php' ) ) . '">WordPress Admin</a>' );
		}
