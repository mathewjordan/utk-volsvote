...
class Native {
	...
	public function init( $query ) {
		...
		if ( $query instanceof \WP_User_Query && empty( trim( $query->get( 'search' ) ) ) ) {
			return;
		} elseif ( ! $query instanceof \WP_User_Query && ! ( $query->is_main_query() && $query->is_search() ) ) {
			return;
		}
	...
	public function find_users( $native_results, \WP_User_Query $query ) {
		...
		add_filter( 'searchwp\query\mods', [ $this, 'limit_engine_to_users' ] );

		$search = new \SearchWP\Query( $query->get( 'search' ), [
			'engine' => $engine->get_name(),
		] );

		remove_filter( 'searchwp\query\mods', [ $this, 'limit_engine_to_users' ] );
	...
	/**
	 * Callback to insert a Mod that limits Query results to Users.
	 *
	 * @since 4.0
	 * @param mixed $mods The incoming Mods.
	 * @return Mod[]
	 */
	public function limit_engine_to_users( $mods ) {
