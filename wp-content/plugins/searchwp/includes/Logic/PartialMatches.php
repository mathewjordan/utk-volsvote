...
class PartialMatches {
	...
	public function find( array $tokens, Query $query ) {
		...
		$keywords_tokens     = $this->tokens->get();
		$this->exact_matches = $this->index->has_tokens( $keywords_tokens );
		$values              = [];
		$excluded            = '1=1';
		...
		if ( ! empty( $partial_tokens ) && ! apply_filters( 'searchwp\query\partial_matches\fuzzy\force', false, [
			...
			return array_merge( $partial_tokens, $this->exact_matches );
		}
	...
	private function get_partial_tokens( array $partials, string $excluded, array $values ) {
		...
		// FUTURE: Do stems need to be considered here, or is that too far of a departure which would lead to unwanted matches?
