...
class Utils {
	 ...
	 * @var string
	 */
	public static $word_match_pattern = '/(?!<.*?)(%s)(?![^<>]*?>)/';
	...
	public static function remove_strings_from_string( array $to_remove, string $string ) {
		...
		// Need to buffer the whitelisted terms to prevent replacement overrun.
	...
	/**
	 * Parses HTML to extract useful bits from the content itself and whitelisted HTML tag attributes.
	 *
	 * @since 4.0
	...
	/**
	 * Getter for token whitelist patterns.
	 *
	 * @since 4.0
	 * @return array
	 */
	public static function get_token_whitelist_patterns() {
		$patterns = apply_filters(
			'searchwp\tokens\whitelist_regex_patterns',
			[
				// Function names.
				"/\b(\\w+?)?\\(|[\\s\\n]\\(/is",
	...
	public static function get_filtered_post__in( array $args = [], $skip_cache = false ) {
		...
		$cache_key = SEARCHWP_PREFIX . 'post__in' . md5( serialize( $args ) );
		$cache     = wp_cache_get( $cache_key, '' );
	...
	public static function get_filtered_post__not_in( array $args = [], $skip_cache = false ) {
		$cache_key = SEARCHWP_PREFIX . 'post__not_in' . md5( serialize( $args ) );
		$cache     = wp_cache_get( $cache_key, '' );
	...
	public static function string_has_substring_from_string( string $string, string $substrings ) {
		$substrings = array_map( 'preg_quote', explode( ' ', $substrings ) );
