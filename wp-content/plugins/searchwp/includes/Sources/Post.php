...
class Post extends Source {
	...
	public function entry( Entry $entry, $doing_query = false ) {
		...
		$global_excerpt = apply_filters( 'searchwp\source\post\global_excerpt', ! empty( $highlighter ) );
		$global_excerpt = apply_filters( 'searchwp\source\post\global_excerpt\\' . $this->post_type, $global_excerpt );
		...
		if ( $doing_query instanceof Query && $highlighter ) {
			$search_terms = explode( ' ', $doing_query->get_keywords() );
	...
	public static function get_global_excerpt( Entry $entry, $query, $length = 55 ) {
		...
		if ( ! empty( $excerpt ) && Utils::string_has_substring_from_string( $excerpt, $search_terms ) ) {
			return Utils::trim_string_around_substring(
				apply_filters( 'searchwp\source\post\excerpt_haystack', $excerpt ),
				$search_terms,
				$length
			);
		}
		...
		$content = strip_tags( html_entity_decode( apply_filters( 'the_content', get_the_content( null, false, $post_id ) ) ) );
		if ( ! empty( $content ) && Utils::string_has_substring_from_string( $content, $search_terms ) ) {
			return Utils::trim_string_around_substring(
				apply_filters( 'searchwp\source\post\excerpt_haystack', $content ),
				$search_terms,
				$length
			);
		}
		...
		if ( ! empty( $entry->get_data()['meta'] ) && is_array( $entry->get_data()['meta'] ) ) {
			foreach ( $entry->get_data()['meta'] as $meta_key => $tokens ) {
				$meta_value = $tokens->raw;
				if ( ! empty( $meta_value ) && Utils::string_has_substring_from_string( $meta_value, $search_terms ) ) {
					...
					$content = strip_tags( html_entity_decode( $meta_value ) );
	...
	public function add_hooks() {
		...
		// Taxonomy changes trigger post updates.
		...
		add_action( 'trash_comment',             [ $this, 'drop_comment_post' ] );
		add_action( 'delete_comment',            [ $this, 'drop_comment_post' ] );
	 ...
	 * @return bool
	 */
	public function comments_in_use() {
		// Comment changes trigger post updates.
	...
	public function transition_comment_status( $new_status, $old_status, $comment ) {
		if ( $new_status === $old_status || ! $this->comments_in_use() ) {
			return;
		}
	...
	public function comment_post( $comment_id, $comment_approved ) {
		if ( 1 !== $comment_approved || ! self::comments_in_use() ) {
			return;
		}
