...
class Indexer extends BackgroundProcess {
	...
	protected function task( $batch ) {
		...
		foreach ( $batch['ids'] as $batch_id => $source_id ) {
			...
			$entry = apply_filters( 'searchwp\indexer\entry', new Entry( $source, $source_id ) );
	...
	public function _destroy_queue() {
		...
		$wpdb->query( $wpdb->prepare( "
			DELETE FROM {$status}
			WHERE indexed IS NULL and queued IS NOT NULL AND site = %d",
			get_current_blog_id()
		) );
	...
	private function check_for_process_failure() {
		...
		$batch_start = get_site_option( $this->identifier . '_process_lock' );
		if ( ! $batch_start ) {
			return;
		}

		// If the current task has been running for more than 45 seconds, consider it a failure.
		list( $usec, $sec ) = explode( ' ', $batch_start );
		$exec_time = time() - absint( (float) $usec + (float) $sec );

		$exec_limit = apply_filters( 'searchwp\indexer\failure_threshold', 45 );

		if ( $exec_time <= absint( $exec_limit ) ) {
			return;
		}
