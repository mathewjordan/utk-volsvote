...
use SearchWP\Index\Tables\IndexTable;
use SearchWP\Index\Tables\TokensTable;
use SearchWP\Index\Tables\StatusTable;
...
class Controller extends BackgroundProcess {
	 ...
	 * @var string
	 */
	protected $action = 'index_controller';
	...
	private function set_tables() {
		$this->tables = [
			...
			'tokens' => new TokensTable(),
			'log'    => new LogTable(),
		];
	}

	/**
	 * Handler for each batch of entries to drop.
	 *
	 * @since 4.0
	 * @param mixed $batch
	...
	protected function task( $batch ) {
		...
		$this->drop( $this->get_source_by_name( $source ), $id, true );
	...
	protected function complete() {
		...
		do_action( 'searchwp\debug\log', 'Delta completed', 'index' );

		// Trigger the Indexer to update.
	...
	/**
	 * Returns Sources that have default values.
	 *
	 * @since 4.0
	 * @return Sources[]
	 */
			...
			foreach ( $source_config->get_attributes() as $attribute ) {
				...
				if (
					...
					if ( ! empty( $options ) && is_array( $options ) ) {
						$settings = call_user_func_array( 'array_merge', array_map( function( $option ) use ( $default ) {
							return [ $option->get_value() => $default ];
						}, $options ) );
					} else {
						$settings = $default;
					}
	...
	public function _add_hooks() {
		...
		if ( ! has_action( SEARCHWP_PREFIX . 'index_controller_dispatch', [ $this, '_dispatch' ] ) ) {
			add_action( SEARCHWP_PREFIX . 'index_controller_dispatch', [ $this, '_dispatch' ] );
		}
	}
	...
	/**
	 * Callback for WP CRON index dispatch.
	 *
	 * @since 4.0
	 */
	public function _dispatch() {
		do_action( 'searchwp\debug\log', 'Dispatching background job', 'index' );
	...
	/**
	 * Triggers a forced cycling of the background process.
	 *
	 * @since 4.0
	 */
	 ...
	 *
	 * @since 4.0
	 * @param Entries $entries The Entries to validate.
	 * @return Entries
	 */
	public function validate_queueable_entries( Entries $entries ) {
		global $wpdb;
	 ...
	 *
	 * @since 4.0
	 * @param Entries $entries The applicable Entries.
	 * @return boolean|int
	 */
	public function introduce( Entries $entries ) {
		global $wpdb;
	...
	public function add( Entry $entry ) {
		...
		foreach ( $entry_data as $attribute => $data ) {
			...
			$this->index_tokenized_entry_attribute_data( $entry, $attribute, $data );
		}

		unset( $entry_data );

		// Lastly mark the entry as officially indexed.
	 ...
	 *
	 * @since 4.0
	 * @param Source     $source The Source to drop.
	 ...
	 * @return boolean
	 */
	public function drop( Source $source, $id, $force = false ) {
		global $wpdb;
		...
		if ( $force || apply_filters( 'searchwp\index\aggressive_delta', false ) ) {
			...
			if ( 'alternate' === $indexer_method ) {
				$indexer->queue_unindexed_entries();
				$indexer->handle_cron_healthcheck( true );
			}
			...
			if (
				! Settings::get( 'indexer_paused', 'boolean' )
				&& apply_filters( 'searchwp\index\process\enabled', true )
			) {
				wp_schedule_single_event( time(), SEARCHWP_PREFIX . 'index_controller_dispatch' );
			} else {
	...
	private function drop_tokens( array $token_ids ) {
		...
		return $wpdb->query( $wpdb->prepare( "
			...
			WHERE id IN ( " . implode( ', ',
				array_fill( 0, count( $token_ids ), '%d' )
			) . ' )',
			$token_ids
		) );
	}
	...
	private function index_tokenized_entry_attribute_data( Entry $entry, string $attribute, Tokens $tokens ) {
		...
		$occurrences = array_count_values( array_map( function( $token ) use ( $token_id_map ) {
	...
	private function get_token_ids( Tokens $tokens ) {
		...
		return $wpdb->get_results( $wpdb->prepare( "
			...
			WHERE token IN ( " . implode( ', ',
				array_fill( 0, count( $tokens->get() ), '%s' )
			) . ' )',
			$tokens->get()
		), ARRAY_A );
	}
	...
	private function reduce_to_unique_tokens( Tokens $tokens ) {
		...
		$existing_tokens = $wpdb->get_col( $wpdb->prepare( "
	...
	private function add_tokens( Tokens $tokens ) {
		...
		$batches = array_chunk(
			$tokens,
			absint( apply_filters( 'searchwp\index\tokens_max', 500 ) )
		);
	 ...
	 */
	public function has_tokens( array $tokens ) {
		global $wpdb;
		...
		return array_map( function( $token ) {
			...
			"SELECT id, token
			FROM {$this->get_tables()['tokens']->table_name}
			WHERE token IN ( " . implode( ', ', array_fill( 0, count( $tokens ), '%s' ) ) . ' )',
			$tokens
		), OBJECT_K ) );
	}
	...
	public function get_stats() {
		...
		return [
			...
			'total'        => $total,
			'omitted'      => $omitted,
			'outdated'     => (bool) Settings::get( 'index_outdated' ),
		];
	}
