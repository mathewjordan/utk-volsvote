...
class Entry {
	...
	/**
	 * Iterates over the Attributes for this Entry Source and retrieves the data for ecah.
	 *
	 * @since 4.0
	 ...
	 * @return void
	 */
	public function update_data( $all_attributes = false ) {
		$this->data = [];
		...
		try {
			foreach ( $this->source->get_attributes() as $attribute ) {
				...
				if ( false === $attribute->get_options() ) {
					$this->data[ $attribute->get_name() ] = $attribute->get_data( $this->id );
					continue;
				}
				...
				if ( ! empty( $chosen_options ) ) {
					...
					foreach ( $chosen_options as $chosen_option ) {
						$attribute_data[ $chosen_option ] = $attribute->get_data( $this->id, $chosen_option );
					}
				}
		...
		$this->data = array_map( function( $attribute_data ) {
			if ( $attribute_data instanceof Tokens ) {
