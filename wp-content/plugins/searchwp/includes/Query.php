...
class Query {
	...
	private function set_mods() {
		...
		// We can't apply this limit when searching an entire network; too much overhead.
		if ( 'all' !== $this->args['site'] && apply_filters( 'searchwp\query\do_source_db_where', true, $this ) ) {
			$this->set_core_mods();
		}
	...
	private function assign_mods_aliases() {
		...
		foreach ( $this->mods as $mod ) {
			...
			if ( ! empty( $raw_join_sql ) ) {
				...
				$raw_join_sql = array_map( function( $clause ) use ( $mod ) {
			...
			if ( in_array( $join_key, $aliases, true ) ) {
				$mod->set_local_table_alias( array_search( $join_key, $aliases ) );
				continue;
			}
	...
	private function get_weight_transfer_clauses( $index_alias = 's' ) {
		...
		$transfers = array_filter( array_map( function( $source ) {
			$source_options = array_filter( $source->get_options(),
		...
		foreach ( $transfers as $transfer ) {
			...
			if ( 'col' === $transfer['transfer']['weight_transfer']['option'] ) {
					...
					if ( 'col' === $option['option']->get_value() ) {
						...
						if ( isset( $option['fallback'] ) ) {
							...
							$fallback = $wpdb->prepare( " AND {$column} NOT IN(" .
								implode( ', ', array_fill( 0, count( $fallback ), '%s' ) ) . ')', $fallback );
						}
				...
				$column = $wpdb->prepare( '%s', $transfer['transfer']['weight_transfer']['value'] );
			}

			$cases[] = $wpdb->prepare( "WHEN {$index_alias}.source = %s {$fallback} THEN {$column}", $transfer['source']->get_name() );
		...
		return [
			'cases' => 'CASE ' . implode( ' ', $cases ) . " ELSE {$index_alias}.id END AS id",
			'joins' => $joins,
		];
	}
	...
	private function build() {
		...
		return (array) apply_filters( 'searchwp\query', [
			...
			'from'     => [
				'select'   => [
					false === $weight_transfers ? "{$index_alias}.id" : $weight_transfers['cases'],
					"{$index_alias}.source",
					"{$index_alias}.site",
	...
	private function index_where() {
		...
		foreach ( $this->mods as $mod ) {
			...
			// If this WHERE clause is a Source it's already using the Source's db_where.
			if ( $where instanceof Source ) {
				continue;
			}

			// If this Mod has a Source then it is not an Index Mod and will be included with the Source WHERE.
			if ( $mod->get_source() ) {
				continue;
			}
	 ...
	 */
	private function get_source_attributes_as_values( array $attributes ) {
		global $wpdb;
	 ...
	 */
	private function get_source_attributes_as_where_sql( array $attributes ) {
		$index_alias = $this->index->get_alias();
		$attributes  = Utils::separate_partial_matches( $attributes);
	...
	private function set_tokens( string $search_string ) {
		...
		$search_string = (string) apply_filters( 'searchwp\query\search_string', $search_string, $this );
		...
		$token_limit = absint( apply_filters( 'searchwp\query\tokens\limit', 100, $this ) );
