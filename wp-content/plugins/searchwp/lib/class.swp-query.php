...
class SWP_Query {
	...
	function __construct( array $args = [] ) {
		...
		$args['s'] = trim( stripslashes( (string) $args['s'] ) );
		$args['s'] = htmlspecialchars_decode( $args['s'] );
		...
		// We're going to JOIN to the posts table.
		$mod = new \SearchWP\Mod();
		$mod->raw_join_sql( "LEFT JOIN {$wpdb->posts} ON ({$wpdb->posts}.ID = s.id)" );
		$this->mods[] = $mod;
	...
	function maybe_orderby() {
		...
		foreach ( $this->orderby as $priority => $orderby ) {
			switch ( $orderby ) {
				case 'date':
					$mod = new \SearchWP\Mod();
					$mod->order_by( "{$wpdb->posts}.post_date", $this->order[ $priority ], $priority + 1 );
	...
	function maybe_tax_query() {
		...
		$tq_sql    = $tax_query->get_sql( $wpdb->posts, 'ID' );
	...
	function maybe_meta_query() {
		...
		$mod = new \SearchWP\Mod();
		$mod->raw_join_sql( $mq_sql['join'] );
		$mod->raw_where_sql( '1=1 ' . $mq_sql['where'] );
	...
	function mods( $mods ) {
		remove_filter( 'searchwp\query\mods', [ $this, 'mods' ], 20 );

		return array_merge( $this->mods, $mods );
	}
	...
	function get_search_results() {
		add_filter( 'searchwp\query\mods', [ $this, 'mods' ], 20 );
