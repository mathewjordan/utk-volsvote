...
class TCPDF_PARSER
{
    ...
    protected function decodeStream($sdic, $stream)
    {
        ...
        foreach ($filters as $filter) {
            if (\in_array($filter, \SearchWP\Dependencies\TCPDF_FILTERS::getAvailableFilters())) {
                try {
                    $stream = \SearchWP\Dependencies\TCPDF_FILTERS::decodeFilter($filter, $stream);
                } catch (\Exception $e) {
                    $emsg = $e->getMessage();
 ...
 * @author Nicola Asuni - info@tecnick.com
 */
\class_alias('SearchWP\\Dependencies\\TCPDF_PARSER', 'TCPDF_PARSER', \false);
