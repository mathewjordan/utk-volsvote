...
class TCPDF_IMAGES
{
    ...
    public static function getImageFileType($imgfile, $iminfo = array())
    {
        ...
        if (empty($type)) {
            ...
            if (isset($fileinfo['extension']) and !\SearchWP\Dependencies\TCPDF_STATIC::empty_string($fileinfo['extension'])) {
                $type = \strtolower(\trim($fileinfo['extension']));
            }
        }
        if ($type == 'jpg') {
            $type = 'jpeg';
    ...
    public static function _parsejpeg($file)
    {
        ...
        if (!@\SearchWP\Dependencies\TCPDF_STATIC::file_exists($file)) {
            return \false;
        }
        $a = \getimagesize($file);
        if (empty($a)) {
        ...
        while (($pos = \strpos($data, "ICC_PROFILE\0", $offset)) !== \false) {
            ...
            $length = \SearchWP\Dependencies\TCPDF_STATIC::_getUSHORT($data, $pos - 2) - 16;
    ...
    public static function _parsepng($file)
    {
        ...
        $w = \SearchWP\Dependencies\TCPDF_STATIC::_freadint($f);
        $h = \SearchWP\Dependencies\TCPDF_STATIC::_freadint($f);
        $bpc = \ord(\fread($f, 1));
        $ct = \ord(\fread($f, 1));
        if ($ct == 0) {
            $colspace = 'DeviceGray';
        ...
        $data = '';
        $icc = \false;
        $n = \SearchWP\Dependencies\TCPDF_STATIC::_freadint($f);
        do {
            $type = \fread($f, 4);
            if ($type == 'PLTE') {
                // read palette
                $pal = \SearchWP\Dependencies\TCPDF_STATIC::rfread($f, $n);
                \fread($f, 4);
            } elseif ($type == 'tRNS') {
                // read transparency info
                $t = \SearchWP\Dependencies\TCPDF_STATIC::rfread($f, $n);
                if ($ct == 0) {
                    // DeviceGray
                    $trns = array(\ord($t[1]));
                } elseif ($ct == 2) {
                ...
                $data .= \SearchWP\Dependencies\TCPDF_STATIC::rfread($f, $n);
                \fread($f, 4);
            } elseif ($type == 'iCCP') {
                // skip profile name
                $len = 0;
                ...
                $icc = \SearchWP\Dependencies\TCPDF_STATIC::rfread($f, $n - $len - 2);
                // decompress profile
                ...
                \SearchWP\Dependencies\TCPDF_STATIC::rfread($f, $n + 4);
            }
            $n = \SearchWP\Dependencies\TCPDF_STATIC::_freadint($f);
        } while ($n);
 ...
 * @author Nicola Asuni - info@tecnick.com
 */
\class_alias('SearchWP\\Dependencies\\TCPDF_IMAGES', 'TCPDF_IMAGES', \false);
