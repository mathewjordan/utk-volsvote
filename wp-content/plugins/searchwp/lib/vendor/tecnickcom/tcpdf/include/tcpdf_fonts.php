...
class TCPDF_FONTS
{
    ...
    {
        if (!\SearchWP\Dependencies\TCPDF_STATIC::file_exists($fontfile)) {
            // Could not find file
            return \false;
        }
        // font metrics
        ...
        if (@\SearchWP\Dependencies\TCPDF_STATIC::file_exists($outpath . $font_name . '.php')) {
        ...
        if (empty($fonttype)) {
            if (\SearchWP\Dependencies\TCPDF_STATIC::_getULONG($font, 0) == 0x10000) {
        ...
        if ($fmetric['type'] == 'TrueType' or $fmetric['type'] == 'Type1') {
            if (!empty($enc) and $enc != 'cp1252' and isset(\SearchWP\Dependencies\TCPDF_FONT_DATA::$encmap[$enc])) {
                // build differences from reference encoding
                $enc_ref = \SearchWP\Dependencies\TCPDF_FONT_DATA::$encmap['cp1252'];
                $enc_target = \SearchWP\Dependencies\TCPDF_FONT_DATA::$encmap[$enc];
                $last = 0;
                for ($i = 32; $i <= 255; ++$i) {
        ...
        if ($fmetric['type'] == 'Type1') {
            ...
            $fmetric['file'] .= '.z';
            $fp = \SearchWP\Dependencies\TCPDF_STATIC::fopenLocal($outpath . $fmetric['file'], 'wb');
            ...
            if (!empty($enc) and isset(\SearchWP\Dependencies\TCPDF_FONT_DATA::$encmap[$enc])) {
                $enc_map = \SearchWP\Dependencies\TCPDF_FONT_DATA::$encmap[$enc];
            } else {
                $enc_map = \false;
            }
            $fmetric['cw'] = '';
            ...
            if (\SearchWP\Dependencies\TCPDF_STATIC::_getULONG($font, $offset) != 0x10000) {
            ...
            $numTables = \SearchWP\Dependencies\TCPDF_STATIC::_getUSHORT($font, $offset);
            $offset += 2;
            ...
            for ($i = 0; $i < $numTables; ++$i) {
                ...
                $offset += 4;
                $table[$tag] = array();
                $table[$tag]['checkSum'] = \SearchWP\Dependencies\TCPDF_STATIC::_getULONG($font, $offset);
                $offset += 4;
                $table[$tag]['offset'] = \SearchWP\Dependencies\TCPDF_STATIC::_getULONG($font, $offset);
                $offset += 4;
                $table[$tag]['length'] = \SearchWP\Dependencies\TCPDF_STATIC::_getULONG($font, $offset);
                $offset += 4;
            }
            // check magicNumber
            $offset = $table['head']['offset'] + 12;
            if (\SearchWP\Dependencies\TCPDF_STATIC::_getULONG($font, $offset) != 0x5f0f3cf5) {
            ...
            // skip flags
            // get FUnits
            $fmetric['unitsPerEm'] = \SearchWP\Dependencies\TCPDF_STATIC::_getUSHORT($font, $offset);
            $offset += 2;
            // units ratio constant
            $urk = 1000 / $fmetric['unitsPerEm'];
            $offset += 16;
            // skip created, modified
            $xMin = \round(\SearchWP\Dependencies\TCPDF_STATIC::_getFWORD($font, $offset) * $urk);
            $offset += 2;
            $yMin = \round(\SearchWP\Dependencies\TCPDF_STATIC::_getFWORD($font, $offset) * $urk);
            $offset += 2;
            $xMax = \round(\SearchWP\Dependencies\TCPDF_STATIC::_getFWORD($font, $offset) * $urk);
            $offset += 2;
            $yMax = \round(\SearchWP\Dependencies\TCPDF_STATIC::_getFWORD($font, $offset) * $urk);
            $offset += 2;
            $fmetric['bbox'] = '' . $xMin . ' ' . $yMin . ' ' . $xMax . ' ' . $yMax . '';
            $macStyle = \SearchWP\Dependencies\TCPDF_STATIC::_getUSHORT($font, $offset);
            $offset += 2;
            // PDF font flags
            $fmetric['Flags'] = $flags;
            if (($macStyle & 2) == 2) {
            ...
            $short_offset = \SearchWP\Dependencies\TCPDF_STATIC::_getSHORT($font, $offset) == 0;
            $offset += 2;
            ...
            if ($short_offset) {
                ...
                for ($i = 0; $i < $tot_num_glyphs; ++$i) {
                    $indexToLoc[$i] = \SearchWP\Dependencies\TCPDF_STATIC::_getUSHORT($font, $offset) * 2;
                ...
                // numGlyphs + 1
                for ($i = 0; $i < $tot_num_glyphs; ++$i) {
                    $indexToLoc[$i] = \SearchWP\Dependencies\TCPDF_STATIC::_getULONG($font, $offset);
                    if (isset($indexToLoc[$i - 1]) && $indexToLoc[$i] == $indexToLoc[$i - 1]) {
                        // the last glyph didn't have an outline
                        unset($indexToLoc[$i - 1]);
                    }
            ...
            $numEncodingTables = \SearchWP\Dependencies\TCPDF_STATIC::_getUSHORT($font, $offset);
            $offset += 2;
            $encodingTables = array();
            for ($i = 0; $i < $numEncodingTables; ++$i) {
                $encodingTables[$i]['platformID'] = \SearchWP\Dependencies\TCPDF_STATIC::_getUSHORT($font, $offset);
                $offset += 2;
                $encodingTables[$i]['encodingID'] = \SearchWP\Dependencies\TCPDF_STATIC::_getUSHORT($font, $offset);
                $offset += 2;
                $encodingTables[$i]['offset'] = \SearchWP\Dependencies\TCPDF_STATIC::_getULONG($font, $offset);
                $offset += 4;
            }
            // ---------- get os/2 metrics ----------
            ...
            // skip version
            // xAvgCharWidth
            $fmetric['AvgWidth'] = \round(\SearchWP\Dependencies\TCPDF_STATIC::_getFWORD($font, $offset) * $urk);
            $offset += 2;
            // usWeightClass
            $usWeightClass = \round(\SearchWP\Dependencies\TCPDF_STATIC::_getUFWORD($font, $offset) * $urk);
            ...
            $offset += 2;
            // usWidthClass
            $fsType = \SearchWP\Dependencies\TCPDF_STATIC::_getSHORT($font, $offset);
            $offset += 2;
            if ($fsType == 2) {
            ...
            $numNameRecords = \SearchWP\Dependencies\TCPDF_STATIC::_getUSHORT($font, $offset);
            $offset += 2;
            // Offset to start of string storage (from start of table).
            $stringStorageOffset = \SearchWP\Dependencies\TCPDF_STATIC::_getUSHORT($font, $offset);
            $offset += 2;
            for ($i = 0; $i < $numNameRecords; ++$i) {
                $offset += 6;
                // skip Platform ID, Platform-specific encoding ID, Language ID.
                // Name ID.
                $nameID = \SearchWP\Dependencies\TCPDF_STATIC::_getUSHORT($font, $offset);
                $offset += 2;
                if ($nameID == 6) {
                    // String length (in bytes).
                    $stringLength = \SearchWP\Dependencies\TCPDF_STATIC::_getUSHORT($font, $offset);
                    $offset += 2;
                    // String offset from start of storage area (in bytes).
                    $stringOffset = \SearchWP\Dependencies\TCPDF_STATIC::_getUSHORT($font, $offset);
                    $offset += 2;
            ...
            $offset += 4;
            // skip Format Type
            $fmetric['italicAngle'] = \SearchWP\Dependencies\TCPDF_STATIC::_getFIXED($font, $offset);
            $offset += 4;
            $fmetric['underlinePosition'] = \round(\SearchWP\Dependencies\TCPDF_STATIC::_getFWORD($font, $offset) * $urk);
            $offset += 2;
            $fmetric['underlineThickness'] = \round(\SearchWP\Dependencies\TCPDF_STATIC::_getFWORD($font, $offset) * $urk);
            $offset += 2;
            $isFixedPitch = \SearchWP\Dependencies\TCPDF_STATIC::_getULONG($font, $offset) == 0 ? \false : \true;
            $offset += 2;
            if ($isFixedPitch) {
                $fmetric['Flags'] |= 1;
            }
            ...
            // Ascender
            $fmetric['Ascent'] = \round(\SearchWP\Dependencies\TCPDF_STATIC::_getFWORD($font, $offset) * $urk);
            $offset += 2;
            // Descender
            $fmetric['Descent'] = \round(\SearchWP\Dependencies\TCPDF_STATIC::_getFWORD($font, $offset) * $urk);
            $offset += 2;
            // LineGap
            $fmetric['Leading'] = \round(\SearchWP\Dependencies\TCPDF_STATIC::_getFWORD($font, $offset) * $urk);
            $offset += 2;
            // advanceWidthMax
            $fmetric['MaxWidth'] = \round(\SearchWP\Dependencies\TCPDF_STATIC::_getUFWORD($font, $offset) * $urk);
            $offset += 2;
            $offset += 22;
            // skip some values
            // get the number of hMetric entries in hmtx table
            $numberOfHMetrics = \SearchWP\Dependencies\TCPDF_STATIC::_getUSHORT($font, $offset);
            // ---------- get maxp data ----------
            $offset = $table['maxp']['offset'];
            $offset += 4;
            // skip Table version number
            // get the the number of glyphs in the font.
            $numGlyphs = \SearchWP\Dependencies\TCPDF_STATIC::_getUSHORT($font, $offset);
            // ---------- get CIDToGIDMap ----------
            $ctg = array();
            foreach ($encodingTables as $enctable) {
                ...
                if ($enctable['platformID'] == $platid and $enctable['encodingID'] == $encid) {
                    ...
                    $format = \SearchWP\Dependencies\TCPDF_STATIC::_getUSHORT($font, $offset);
                    $offset += 2;
                    switch ($format) {
                        case 0:
                            ...
                            for ($c = 0; $c < 256; ++$c) {
                                $g = \SearchWP\Dependencies\TCPDF_STATIC::_getBYTE($font, $offset);
                                $ctg[$c] = $g;
                                ++$offset;
                            }
                            break;
                        case 2:
                            ...
                            for ($i = 0; $i < 256; ++$i) {
                                ...
                                $subHeaderKeys[$i] = \SearchWP\Dependencies\TCPDF_STATIC::_getUSHORT($font, $offset) / 8;
                                $offset += 2;
                            ...
                            for ($k = 0; $k < $numSubHeaders; ++$k) {
                                $subHeaders[$k]['firstCode'] = \SearchWP\Dependencies\TCPDF_STATIC::_getUSHORT($font, $offset);
                                $offset += 2;
                                $subHeaders[$k]['entryCount'] = \SearchWP\Dependencies\TCPDF_STATIC::_getUSHORT($font, $offset);
                                $offset += 2;
                                $subHeaders[$k]['idDelta'] = \SearchWP\Dependencies\TCPDF_STATIC::_getUSHORT($font, $offset);
                                $offset += 2;
                                $subHeaders[$k]['idRangeOffset'] = \SearchWP\Dependencies\TCPDF_STATIC::_getUSHORT($font, $offset);
                                $offset += 2;
                            ...
                            for ($k = 0; $k < $numGlyphIndexArray; ++$k) {
                                $glyphIndexArray[$k] = \SearchWP\Dependencies\TCPDF_STATIC::_getUSHORT($font, $offset);
                                $offset += 2;
                            }
                            for ($i = 0; $i < 256; ++$i) {
                                $k = $subHeaderKeys[$i];
                        ...
                        case 4:
                            ...
                            $length = \SearchWP\Dependencies\TCPDF_STATIC::_getUSHORT($font, $offset);
                            $offset += 2;
                            $offset += 2;
                            // skip version/language
                            $segCount = \floor(\SearchWP\Dependencies\TCPDF_STATIC::_getUSHORT($font, $offset) / 2);
                            $offset += 2;
                            $offset += 6;
                            ...
                            for ($k = 0; $k < $segCount; ++$k) {
                                $endCount[$k] = \SearchWP\Dependencies\TCPDF_STATIC::_getUSHORT($font, $offset);
                                $offset += 2;
                            }
                            $offset += 2;
                            // skip reservedPad
                            ...
                            for ($k = 0; $k < $segCount; ++$k) {
                                $startCount[$k] = \SearchWP\Dependencies\TCPDF_STATIC::_getUSHORT($font, $offset);
                                $offset += 2;
                            }
                            $idDelta = array();
                            // delta for all character codes in segment
                            for ($k = 0; $k < $segCount; ++$k) {
                                $idDelta[$k] = \SearchWP\Dependencies\TCPDF_STATIC::_getUSHORT($font, $offset);
                                $offset += 2;
                            }
                            $idRangeOffset = array();
                            // Offsets into glyphIdArray or 0
                            for ($k = 0; $k < $segCount; ++$k) {
                                $idRangeOffset[$k] = \SearchWP\Dependencies\TCPDF_STATIC::_getUSHORT($font, $offset);
                                $offset += 2;
                            }
                            ...
                            // glyph index array
                            for ($k = 0; $k < $gidlen; ++$k) {
                                $glyphIdArray[$k] = \SearchWP\Dependencies\TCPDF_STATIC::_getUSHORT($font, $offset);
                                $offset += 2;
                            }
                        ...
                        case 6:
                            ...
                            $offset += 4;
                            // skip length and version/language
                            $firstCode = \SearchWP\Dependencies\TCPDF_STATIC::_getUSHORT($font, $offset);
                            $offset += 2;
                            $entryCount = \SearchWP\Dependencies\TCPDF_STATIC::_getUSHORT($font, $offset);
                            $offset += 2;
                            for ($k = 0; $k < $entryCount; ++$k) {
                                $c = $k + $firstCode;
                                $g = \SearchWP\Dependencies\TCPDF_STATIC::_getUSHORT($font, $offset);
                                $offset += 2;
                                $ctg[$c] = $g;
                            }
                            break;
                        case 8:
                            ...
                            for ($k = 0; $k < 8192; ++$k) {
                                $is32[$k] = \SearchWP\Dependencies\TCPDF_STATIC::_getBYTE($font, $offset);
                                ++$offset;
                            }
                            $nGroups = \SearchWP\Dependencies\TCPDF_STATIC::_getULONG($font, $offset);
                            $offset += 4;
                            for ($i = 0; $i < $nGroups; ++$i) {
                                $startCharCode = \SearchWP\Dependencies\TCPDF_STATIC::_getULONG($font, $offset);
                                $offset += 4;
                                $endCharCode = \SearchWP\Dependencies\TCPDF_STATIC::_getULONG($font, $offset);
                                $offset += 4;
                                $startGlyphID = \SearchWP\Dependencies\TCPDF_STATIC::_getULONG($font, $offset);
                                $offset += 4;
                        ...
                        case 10:
                            ...
                            $startCharCode = \SearchWP\Dependencies\TCPDF_STATIC::_getULONG($font, $offset);
                            $offset += 4;
                            $numChars = \SearchWP\Dependencies\TCPDF_STATIC::_getULONG($font, $offset);
                            $offset += 4;
                        ...
                        case 12:
                            ...
                            for ($k = 0; $k < $nGroups; ++$k) {
                                ...
                                $endCharCode = \SearchWP\Dependencies\TCPDF_STATIC::_getULONG($font, $offset);
                                $offset += 4;
                                $startGlyphCode = \SearchWP\Dependencies\TCPDF_STATIC::_getULONG($font, $offset);
                                $offset += 4;
            ...
            $yMin = \SearchWP\Dependencies\TCPDF_STATIC::_getFWORD($font, $offset);
            $offset += 4;
            $yMax = \SearchWP\Dependencies\TCPDF_STATIC::_getFWORD($font, $offset);
            $offset += 2;
            ...
            for ($i = 0; $i < $numberOfHMetrics; ++$i) {
                $cw[$i] = \round(\SearchWP\Dependencies\TCPDF_STATIC::_getUFWORD($font, $offset) * $urk);
                $offset += 4;
                // skip lsb
            }
            ...
            for ($cid = 0; $cid <= 65535; ++$cid) {
                if (isset($ctg[$cid])) {
                    ...
                    if ($addcbbox and isset($indexToLoc[$ctg[$cid]])) {
                        ...
                        $xMin = \round(\SearchWP\Dependencies\TCPDF_STATIC::_getFWORD($font, $offset + 2) * $urk);
                        $yMin = \round(\SearchWP\Dependencies\TCPDF_STATIC::_getFWORD($font, $offset + 4) * $urk);
                        $xMax = \round(\SearchWP\Dependencies\TCPDF_STATIC::_getFWORD($font, $offset + 6) * $urk);
                        $yMax = \round(\SearchWP\Dependencies\TCPDF_STATIC::_getFWORD($font, $offset + 8) * $urk);
        ...
        if ($fmetric['type'] == 'Type1') {
            ...
            if ($fmetric['type'] == 'cidfont0') {
                ...
                $fp = \SearchWP\Dependencies\TCPDF_STATIC::fopenLocal($outpath . $fmetric['ctg'], 'wb');
        ...
        $pfile .= '// --- EOF ---' . "\n";
        // store file
        $fp = \SearchWP\Dependencies\TCPDF_STATIC::fopenLocal($outpath . $font_name . '.php', 'w');
        \fwrite($fp, $pfile);
        \fclose($fp);
        ...
        while (!empty($new_sga)) {
            ...
            foreach ($sga as $key => $val) {
                if (isset($indexToLoc[$key])) {
                    ...
                    $numberOfContours = \SearchWP\Dependencies\TCPDF_STATIC::_getSHORT($font, $offset);
                    $offset += 2;
                    if ($numberOfContours < 0) {
                        ...
                        do {
                            $flags = \SearchWP\Dependencies\TCPDF_STATIC::_getUSHORT($font, $offset);
                            $offset += 2;
                            $glyphIndex = \SearchWP\Dependencies\TCPDF_STATIC::_getUSHORT($font, $offset);
                            $offset += 2;
    ...
    public static function getFontFullPath($file, $fontdir = \false)
    {
        ...
        if ($fontdir !== \false and @\SearchWP\Dependencies\TCPDF_STATIC::file_exists($fontdir . $file)) {
            $fontfile = $fontdir . $file;
        } elseif (@\SearchWP\Dependencies\TCPDF_STATIC::file_exists(self::_getfontpath() . $file)) {
            $fontfile = self::_getfontpath() . $file;
        } elseif (@\SearchWP\Dependencies\TCPDF_STATIC::file_exists($file)) {
            $fontfile = $file;
        }
        return $fontfile;
    }
    ...
    public static function UTF8ArrToLatin1Arr($unicode)
    {
        ...
        foreach ($unicode as $char) {
            if ($char < 256) {
                $outarr[] = $char;
            } elseif (\array_key_exists($char, \SearchWP\Dependencies\TCPDF_FONT_DATA::$uni_utf8tolatin)) {
                // map from UTF-8
                $outarr[] = \SearchWP\Dependencies\TCPDF_FONT_DATA::$uni_utf8tolatin[$char];
            } elseif ($char == 0xfffd) {
                // skip
            } else {
                $outarr[] = 63;
    ...
    public static function UTF8ArrToLatin1($unicode)
    {
        ...
        foreach ($unicode as $char) {
            if ($char < 256) {
                ...
                // map from UTF-8
                $outstr .= \chr(\SearchWP\Dependencies\TCPDF_FONT_DATA::$uni_utf8tolatin[$char]);
            } elseif ($char == 0xfffd) {
                // skip
            } else {
                $outstr .= '?';
    ...
    public static function UTF8StringToArray($str, $isunicode = \true, &$currentfont)
    {
        if ($isunicode) {
            ...
            $chars = \SearchWP\Dependencies\TCPDF_STATIC::pregSplit('//', 'u', $str, -1, \PREG_SPLIT_NO_EMPTY);
    ...
    public static function utf8Bidi($ta, $str = '', $forcertl = \false, $isunicode = \true, &$currentfont)
    {
        ...
        // max level
        $maxlevel = 0;
        if (\SearchWP\Dependencies\TCPDF_STATIC::empty_string($str)) {
            // create string from array
            $str = self::UTF8ArrSubString($ta, '', '', $isunicode);
        }
        // check if string contains arabic text
        if (\preg_match(\SearchWP\Dependencies\TCPDF_FONT_DATA::$uni_RE_PATTERN_ARABIC, $str)) {
            $arabic = \true;
        } else {
            $arabic = \false;
        }
        // check if string contains RTL text
        if (!($forcertl or $arabic or \preg_match(\SearchWP\Dependencies\TCPDF_FONT_DATA::$uni_RE_PATTERN_RTL, $str))) {
            return $ta;
        }
        // get number of chars
        $numchars = \count($ta);
        if ($forcertl == 'R') {
            ...
            for ($i = 0; $i < $numchars; ++$i) {
                $type = \SearchWP\Dependencies\TCPDF_FONT_DATA::$uni_type[$ta[$i]];
                if ($type == 'L') {
                    $pel = 0;
                    break;
        ...
        for ($i = 0; $i < $numchars; ++$i) {
            if ($ta[$i] == \SearchWP\Dependencies\TCPDF_FONT_DATA::$uni_RLE) {
                // X2. With each RLE, compute the least greater odd embedding level.
                //	a. If this new level would be valid, then this embedding code is valid. Remember (push) the current embedding level and override status. Reset the current level to this new level, and reset the override status to neutral.
                //	b. If the new level would not be valid, then this code is invalid. Do not change the current level or override status.
                $next_level = $cel + $cel % 2 + 1;
                if ($next_level < 62) {
                    $remember[] = array('num' => \SearchWP\Dependencies\TCPDF_FONT_DATA::$uni_RLE, 'cel' => $cel, 'dos' => $dos);
                    $cel = $next_level;
                    $dos = 'N';
                    $sor = $eor;
                    $eor = $cel % 2 ? 'R' : 'L';
                }
            } elseif ($ta[$i] == \SearchWP\Dependencies\TCPDF_FONT_DATA::$uni_LRE) {
                // X3. With each LRE, compute the least greater even embedding level.
                //	a. If this new level would be valid, then this embedding code is valid. Remember (push) the current embedding level and override status. Reset the current level to this new level, and reset the override status to neutral.
                //	b. If the new level would not be valid, then this code is invalid. Do not change the current level or override status.
                $next_level = $cel + 2 - $cel % 2;
                if ($next_level < 62) {
                    $remember[] = array('num' => \SearchWP\Dependencies\TCPDF_FONT_DATA::$uni_LRE, 'cel' => $cel, 'dos' => $dos);
                    $cel = $next_level;
                    $dos = 'N';
                    $sor = $eor;
                    $eor = $cel % 2 ? 'R' : 'L';
                }
            } elseif ($ta[$i] == \SearchWP\Dependencies\TCPDF_FONT_DATA::$uni_RLO) {
                // X4. With each RLO, compute the least greater odd embedding level.
                //	a. If this new level would be valid, then this embedding code is valid. Remember (push) the current embedding level and override status. Reset the current level to this new level, and reset the override status to right-to-left.
                //	b. If the new level would not be valid, then this code is invalid. Do not change the current level or override status.
                $next_level = $cel + $cel % 2 + 1;
                if ($next_level < 62) {
                    $remember[] = array('num' => \SearchWP\Dependencies\TCPDF_FONT_DATA::$uni_RLO, 'cel' => $cel, 'dos' => $dos);
                    $cel = $next_level;
                    $dos = 'R';
                    $sor = $eor;
                    $eor = $cel % 2 ? 'R' : 'L';
                }
            } elseif ($ta[$i] == \SearchWP\Dependencies\TCPDF_FONT_DATA::$uni_LRO) {
                // X5. With each LRO, compute the least greater even embedding level.
                //	a. If this new level would be valid, then this embedding code is valid. Remember (push) the current embedding level and override status. Reset the current level to this new level, and reset the override status to left-to-right.
                //	b. If the new level would not be valid, then this code is invalid. Do not change the current level or override status.
                $next_level = $cel + 2 - $cel % 2;
                if ($next_level < 62) {
                    $remember[] = array('num' => \SearchWP\Dependencies\TCPDF_FONT_DATA::$uni_LRO, 'cel' => $cel, 'dos' => $dos);
                    $cel = $next_level;
                    $dos = 'L';
                    $sor = $eor;
                    $eor = $cel % 2 ? 'R' : 'L';
                }
            } elseif ($ta[$i] == \SearchWP\Dependencies\TCPDF_FONT_DATA::$uni_PDF) {
                // X7. With each PDF, determine the matching embedding or override code. If there was a valid matching code, restore (pop) the last remembered (pushed) embedding level and directional override.
                if (\count($remember)) {
                    $last = \count($remember) - 1;
                    if ($remember[$last]['num'] == \SearchWP\Dependencies\TCPDF_FONT_DATA::$uni_RLE or $remember[$last]['num'] == \SearchWP\Dependencies\TCPDF_FONT_DATA::$uni_LRE or $remember[$last]['num'] == \SearchWP\Dependencies\TCPDF_FONT_DATA::$uni_RLO or $remember[$last]['num'] == \SearchWP\Dependencies\TCPDF_FONT_DATA::$uni_LRO) {
            ...
            } elseif ($ta[$i] != \SearchWP\Dependencies\TCPDF_FONT_DATA::$uni_RLE and $ta[$i] != \SearchWP\Dependencies\TCPDF_FONT_DATA::$uni_LRE and $ta[$i] != \SearchWP\Dependencies\TCPDF_FONT_DATA::$uni_RLO and $ta[$i] != \SearchWP\Dependencies\TCPDF_FONT_DATA::$uni_LRO and $ta[$i] != \SearchWP\Dependencies\TCPDF_FONT_DATA::$uni_PDF) {
                ...
                if ($dos != 'N') {
                    ...
                    if (isset(\SearchWP\Dependencies\TCPDF_FONT_DATA::$uni_type[$ta[$i]])) {
                        $chardir = \SearchWP\Dependencies\TCPDF_FONT_DATA::$uni_type[$ta[$i]];
                    } else {
                        $chardir = 'L';
                    }
                }
        ...
        if ($arabic) {
            ...
            $x = 0;
            for ($i = 0; $i < $numchars; ++$i) {
                if (\SearchWP\Dependencies\TCPDF_FONT_DATA::$uni_type[$chardata[$i]['char']] == 'AL' or $chardata[$i]['char'] == 32 or $chardata[$i]['char'] == 8204) {
                    $charAL[$x] = $chardata[$i];
                    $charAL[$x]['i'] = $i;
                    $chardata[$i]['x'] = $x;
                    ++$x;
            ...
            for ($i = 0; $i < $numchars; ++$i) {
                ...
                if (\SearchWP\Dependencies\TCPDF_FONT_DATA::$uni_type[$thischar['char']] == 'AL') {
                    $x = $thischar['x'];
                    if ($x > 0) {
                        $prevchar = $charAL[$x - 1];
                    } else {
                    ...
                    if ($prevchar !== \false and $prevchar['char'] == 1604 and \in_array($thischar['char'], $alfletter)) {
                        $arabicarr = \SearchWP\Dependencies\TCPDF_FONT_DATA::$uni_laa_array;
                        $laaletter = \true;
                        if ($x > 1) {
                            $prevchar = $charAL[$x - 2];
                        } else {
                        ...
                        $arabicarr = \SearchWP\Dependencies\TCPDF_FONT_DATA::$uni_arabicsubst;
                        $laaletter = \false;
                    }
                    if ($prevchar !== \false and $nextchar !== \false and (\SearchWP\Dependencies\TCPDF_FONT_DATA::$uni_type[$prevchar['char']] == 'AL' or \SearchWP\Dependencies\TCPDF_FONT_DATA::$uni_type[$prevchar['char']] == 'NSM') and (\SearchWP\Dependencies\TCPDF_FONT_DATA::$uni_type[$nextchar['char']] == 'AL' or \SearchWP\Dependencies\TCPDF_FONT_DATA::$uni_type[$nextchar['char']] == 'NSM') and $prevchar['type'] == $thischar['type'] and $nextchar['type'] == $thischar['type'] and $nextchar['char'] != 1567) {
                    ...
                    } elseif ($nextchar !== \false and (\SearchWP\Dependencies\TCPDF_FONT_DATA::$uni_type[$nextchar['char']] == 'AL' or \SearchWP\Dependencies\TCPDF_FONT_DATA::$uni_type[$nextchar['char']] == 'NSM') and $nextchar['type'] == $thischar['type'] and $nextchar['char'] != 1567) {
                        if (isset($arabicarr[$chardata[$i]['char']][2])) {
                            // initial
                            $chardata2[$i]['char'] = $arabicarr[$thischar['char']][2];
                        }
                    } elseif ($prevchar !== \false and (\SearchWP\Dependencies\TCPDF_FONT_DATA::$uni_type[$prevchar['char']] == 'AL' or \SearchWP\Dependencies\TCPDF_FONT_DATA::$uni_type[$prevchar['char']] == 'NSM') and $prevchar['type'] == $thischar['type'] or $nextchar !== \false and $nextchar['char'] == 1567) {
                        // final
            ...
            for ($i = 0; $i < $numchars - 1; ++$i) {
                if ($chardata2[$i]['char'] == 1617 and isset(\SearchWP\Dependencies\TCPDF_FONT_DATA::$uni_diacritics[$chardata2[$i + 1]['char']])) {
                    // check if the subtitution font is defined on current font
                    if (isset($currentfont['cw'][\SearchWP\Dependencies\TCPDF_FONT_DATA::$uni_diacritics[$chardata2[$i + 1]['char']]])) {
                        $chardata2[$i]['char'] = \false;
                        $chardata2[$i + 1]['char'] = \SearchWP\Dependencies\TCPDF_FONT_DATA::$uni_diacritics[$chardata2[$i + 1]['char']];
                    }
                }
            }
        ...
        for ($j = $maxlevel; $j > 0; $j--) {
            ...
            for ($i = 0; $i < $numchars; ++$i) {
                if ($chardata[$i]['level'] >= $j) {
                    $onlevel = \true;
                    if (isset(\SearchWP\Dependencies\TCPDF_FONT_DATA::$uni_mirror[$chardata[$i]['char']])) {
                        // L4. A character is depicted by a mirrored glyph if and only if (a) the resolved directionality of that character is R, and (b) the Bidi_Mirrored property value of that character is true.
                        $chardata[$i]['char'] = \SearchWP\Dependencies\TCPDF_FONT_DATA::$uni_mirror[$chardata[$i]['char']];
                    }
                    $revarr[] = $chardata[$i];
                } else {
                    if ($onlevel) {
 ...
 * @author Nicola Asuni - info@tecnick.com
 */
\class_alias('SearchWP\\Dependencies\\TCPDF_FONTS', 'TCPDF_FONTS', \false);
