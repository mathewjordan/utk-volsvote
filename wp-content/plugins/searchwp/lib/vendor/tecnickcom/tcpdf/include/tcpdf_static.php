...
class TCPDF_STATIC
{
    ...
    {
        return \tempnam(\K_PATH_CACHE, '__tcpdf_' . $file_id . '_' . $type . '_' . \md5(\SearchWP\Dependencies\TCPDF_STATIC::getRandomSeed()) . '_');
    }
    /**
     * Add "\" before "\", "(" and ")"
    ...
    public static function getAnnotOptFromJSProp($prop, &$spot_colors, $rtl = \false)
    {
        ...
        if (isset($prop['fillColor'])) {
            if (\is_array($prop['fillColor'])) {
                ...
                $opt['mk']['bg'] = \SearchWP\Dependencies\TCPDF_COLORS::convertHTMLColorToDec($prop['fillColor'], $spot_colors);
            }
        }
        ...
        if (isset($prop['strokeColor'])) {
            if (\is_array($prop['strokeColor'])) {
                ...
                $opt['mk']['bc'] = \SearchWP\Dependencies\TCPDF_COLORS::convertHTMLColorToDec($prop['strokeColor'], $spot_colors);
            }
        }
    ...
    public static function extractCSSproperties($cssdata)
    {
        ...
        $cssblocks = array();
        $matches = array();
        if (\preg_match_all('/@media[\\s]+([^\\Â§]*)Â§([^Â§]*)Â§/i', $cssdata, $matches) > 0) {
    ...
    public static function getHyphenPatternsFromTEX($file)
    {
        ...
        foreach ($patterns_array as $val) {
            if (!\SearchWP\Dependencies\TCPDF_STATIC::empty_string($val)) {
                $val = \trim($val);
                $val = \str_replace('\'', '\\\'', $val);
 ...
 * @author Nicola Asuni - info@tecnick.com
 */
\class_alias('SearchWP\\Dependencies\\TCPDF_STATIC', 'TCPDF_STATIC', \false);
