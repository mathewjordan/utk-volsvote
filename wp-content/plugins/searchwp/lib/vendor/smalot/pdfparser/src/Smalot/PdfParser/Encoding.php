...
class Encoding extends \SearchWP\Dependencies\Smalot\PdfParser\PDFObject
{
    ...
    public function init()
    {
        ...
        if ($this->has('BaseEncoding')) {
            ...
            $table = \array_flip(\array_reverse($this->encoding, \true));
            foreach ($this->differences as $code => $difference) {
                /** @var string $difference */
                $this->mapping[$code] = isset($table[$difference]) ? $table[$difference] : \SearchWP\Dependencies\Smalot\PdfParser\Font::MISSING;
            }
        }
    }
    /**
