...
require_once( dirname( __FILE__ ) . '/bsshort/defaults.php' );
require_once( dirname( __FILE__ ) . '/bsshort/functions.php' );
require_once( dirname( __FILE__ ) . '/bsshort/actions-filters.php' );
...
class BoostrapShortcodes {
  ...
  function bs_button($atts, $content = null) {
     ...
     $return .= ($color) ? ' btn-' . $color : ' btn-default';
  ...
  */
  ...
  function bs_tabs( $atts, $content = null ) {
    ...
    if ( $atts_map ) {
      ...
      foreach( $atts_map as $tab ) {
        ...
        $tabs[] = sprintf(
          '<li%s  role="presentation" ><a href="#%s"  role="tab" data-toggle="tab">%s</a></li>',
  ...
  function bs_collapsibles( $atts, $content = null ) {
    ...
    return sprintf( 
      '<div class="%s" id="%s"%s role="tablist" aria-multiselectable="true">%s</div>',
      esc_attr( $class ),
      esc_attr( $id ),
      ( $data_props ) ? ' ' . $data_props : '',
      do_shortcode( $content )
  ...
  function bs_collapse( $atts, $content = null ) {
    ...
    return sprintf( 
      '<div class="panel  %1$s"%2$s><div class="panel-heading"><h3 class="panel-title"><a class="%3$s" data-toggle="collapse"%4$s href="#%5$s"  aria-expanded="true" aria-controls="#%5$s"><svg xmlns="http://www.w3.org/2000/svg" class="icon-collapse" viewBox="0 0 24 24"><path d="M0 7.33l2.829-2.83 9.175 9.339 9.167-9.339 2.829 2.83-11.996 12.17z"/></svg> %6$s</a></h3></div><div id="%5$s" class="panel-collapse collapse %7$s"><div class="panel-body">%8$s</div></div></div>',
