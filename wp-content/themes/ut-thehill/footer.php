...
<div id="linkdrawer" 
<?php utthehill_footer_sidebar_class(); ?>
> 
...
<?php utthehill_expandedfooter_sidebar_class(); ?>
	> 
	<div class="container">
...
?>
		<div id="meta-info">
			<p>
<?php if($title && $title != "") {
						echo "<strong class=\"sitetitle\">" . $title . "</strong><br>";
					}
					?>
<?php


			if ($meta_parentunit_name!="") { ?>
<?php if ($meta_parentunit_link!="") { ?>
				<a href="<?php echo $meta_parentunit_link; ?>"> 
<?php } ?>
				<em><?php echo $meta_parentunit_name; ?>
				</em> 
<?php if ($meta_parentunit_link!="") { ?>
<?php } ?>
				</a> 
...
<?php
						if($contactinfo_map != "" && $contactinfo_address != "") {
							echo "<a href=\"".$contactinfo_map."\"><svg class=\"meta-address\" xmlns=\"http://www.w3.org/2000/svg\" width=\"24\" height=\"24\" viewBox=\"0 0 24 24\"><path d=\"M12 0c-4.198 0-8 3.403-8 7.602 0 4.198 3.469 9.21 8 16.398 4.531-7.188 8-12.2 8-16.398 0-4.199-3.801-7.602-8-7.602zm0 11c-1.657 0-3-1.343-3-3s1.343-3 3-3 3 1.343 3 3-1.343 3-3 3z\"/></svg>";
						}
						...
						if($contactinfo_general != "") {
							echo $contactinfo_general."<br>";
						}


						if($contactinfo_phone1label != "") {
							echo $contactinfo_phone1label.': ';
						}
						if($contactinfo_phone1number != "") {
							echo '<a href="tel:+1'.$contactinfo_phone1numberStripped.'" class="tel">'.$contactinfo_phone1number.'</a>';
						}

						if($contactinfo_phone2label != "") {
							echo ' <span class="bg-scr">&bull;</span><span class="sm-scr"></span> '.$contactinfo_phone2label.': ';
						}
						if($contactinfo_phone2number != "") {
							echo '<a href="tel:+1'.$contactinfo_phone2numberStripped.'" class="tel">'.$contactinfo_phone2number.'</a>';
						}

						if($contactinfo_phone3label != "") {
							echo ' <span class="bg-scr">&bull;</span><span class="sm-scr"></span> '.$contactinfo_phone3label.': ';
						}
						if($contactinfo_phone3number != "") {
							echo '<a href="tel:+1'.$contactinfo_phone3labelStripped.'" class="tel">'.$contactinfo_phone3label.'</a>';
						}
...
<?php
						if($contactinfo_email1label != "") {
							echo '<br>'.$contactinfo_email1label.':&nbsp;'?>
				<a href="mailto:<?php echo $contactinfo_email1address; ?>"><?php echo $contactinfo_email1address; ?>
				</a> 
<?php } ?>
				</span>
			</p>
...
//	datumTokenizer: Bloodhound.tokenizers.obj.whitespace('post_title'),
	...
	remote: {
		url: '<?php echo esc_url( home_url( '/' ) ); ?>wp-json/ja/v2/search?term=%QUERY',
		wildcard: '%QUERY'
	}
});
...
//	limit: 5,
	...
	templates:{
		...
		suggestion: Handlebars.compile('<div class="searchListItem"><a href="<?php echo esc_url( home_url( '/' ) ); ?>?p={{ID}}" class="searchLink" aria-label="{{post_title}}"><span class="suggestion-head">{{post_title}}</span></a></div>')
	}
}).on('typeahead:selected', function(event, selection) {
	url = "<?php echo esc_url( home_url( '/' ) ); ?>
			" + "?p=" + (selection.ID); window.location.href = url; }); 
			</script>
			</body>
			</html>